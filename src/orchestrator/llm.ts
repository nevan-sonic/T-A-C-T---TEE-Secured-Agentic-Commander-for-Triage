...
Respond with JSON only:
{
  ...
  "patch": "complete corrected file content. If the target file is a JSON file (e.g. package.json), output raw, clean JSON with absolutely no comment wrappers (such as /* ... */ or //). Otherwise, output standard clean source code.",
...
Affected file content: ${cveAlert.vulnerableCode || "not provided"}`;
        ...
        if (parsed.patch && typeof parsed.patch === "object") {
            parsed.patch = JSON.stringify(parsed.patch, null, 2);
        }
        console.log(`[Groq LLM] CVE analysis complete for ${cveAlert.cveId}.`);
        return parsed as CVEAnalysisResult;
    } catch (e: any) {
