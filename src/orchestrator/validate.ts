...
function checkSyntax(patch: string, filename?: string): boolean {
    if (filename && (filename.endsWith(".json") || filename === "package.json")) {
        // Explicitly parse JSON files as JSON only, and forbid JavaScript comments
        if (patch.includes("/*") || patch.includes("*/") || patch.includes("//")) {
            console.log(`[Validate] Rejecting JSON patch for '${filename}' because it contains JS comment syntax.`);
            return false;
        }
        try {
            JSON.parse(patch);
 ...
 * @param filename - The target filename being patched
 */
export function validatePatch(
    ...
    if (patch && typeof patch !== "string") {
        try {
            ...
            return {
                score: 0,
                safe: false,
                reason: "Invalid patch format (non-string and not serializable)",
    ...
    const hasSyntax = checkSyntax(patch, filename);
